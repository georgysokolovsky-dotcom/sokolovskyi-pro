...
function RouteMeta({ path }) {
  ...
  useEffect(() => {
    ...
    const articleImage = article ? `${siteOrigin}/images/social/${article.id}-social.svg` : `${siteOrigin}${portraitPath}`;
    const shouldNoindex = legalPaths.includes(path) || path.startsWith('/cases');
    ...
    ogImage?.setAttribute('content', articleImage);
    ogImageAlt?.setAttribute('content', article ? article.title : 'Георгий Соколовский — ментор для мужчин');
